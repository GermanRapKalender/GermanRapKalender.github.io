...
namespace GermanRapKalenderUI
{
	...
	static class Globals
	{
		...
		public static void Init()
		{
			...
			tmp.Add("-");
			tmp.Add("-");
			tmp.Add(" === DIRM Started (Version: '" + Globals.ProjectVersion + "' BuildInfo: '" + Globals.BuildInfo + "' Built at: '" + MyCreationDate + "' Central European Time) ===");
			tmp.Add("    Time Now: '" + DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss") + "'");
			tmp.Add("    Time Now UTC: '" + DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ss") + "'");
			tmp.Add("Logging initiated. Time to the left is local time and NOT UTC. See Debug for more Info");

			Logger.Init(Globals.Logfile, tmp.ToArray());
		...
		{
			if (!FileHandling.doesPathExist(Globals.CSVSubfolderPath))
			{
				FileHandling.createPath(Globals.CSVSubfolderPath);
				if (!FileHandling.doesPathExist(Globals.CSVSubfolderPath))
				{
		...
		public static string XML_AutoUpdate
		{
			get
			{
				...
				return FileHandling.GetStringFromURL(masterURL);
			}
		}
		...
		public static void CheckForUpdate(string XML_Autoupdate_Temp = "")
		{
			...
			string MyVersionOnlineString = FileHandling.GetXMLTagContent(XML_Autoupdate_Temp, "version");
			...
			while (true)
			{
				...
				if (!(String.IsNullOrEmpty(MyVersionOnlineString)))
				{
					...
					// Logging some stuff
					Logger.Log("Checking for DIRM Update during start up procedure");
					Logger.Log("MyVersionOnline = '" + MyVersionOnline.ToString() + "', Globals.ProjectVersion = '" + Globals.ProjectVersion + "'", 1);

					// If Online Version is "bigger" than our own local Version
					if (MyVersionOnline > Globals.ProjectVersion)
					{
						// Update Found.
						Logger.Log("Update found (Version Check returning true).", 1);
						Logger.Log("Checking if URL is reachable.", 1);

						string DLPath = FileHandling.GetXMLTagContent(XML_Autoupdate_Temp, "url");
						...
						{
							// User wants Update
							Logger.Log("Presented Update Choice to User. User wants it", 1);

							FileHandling.deleteFile(LocalFileName);
							new Popups.PopupDownload(DLPath, LocalFileName, "Installer").ShowDialog();

							if (FileHandling.GetSizeOfFile(LocalFileName) > 1000)
							{
								Logger.Log("Installer on Disk looks good, Starting Installer, Exits this now.", 1);
								Process.Start(LocalFileName);
								Environment.Exit(0);
							}
							else
							{
								Logger.Log("Installer on Disk does not look good. FileSize: '" + FileHandling.GetSizeOfFile(LocalFileName) + "'. Will not start it. Will not exit this.", 1);
							}
						}
						else
						{
							// User doesnt want update
							Logger.Log("User does not wants update", 1);
						}
					}
					else
					{
				...
				else
				{
					...
					Logger.Log("Did not get most up to date DIRM Version from Github. Github offline or your PC offline. Probably. Lets hope so.");
				}
				break;
			}
		}
